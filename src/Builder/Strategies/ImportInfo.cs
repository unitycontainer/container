...
namespace Unity.Container
{
    ...
    public struct ImportInfo<TMember> : IInjectionInfo<TMember>
    {
        #region Fields
        ...
        static ImportInfo()
        {
            switch (typeof(TMember))
            {
                ...
                case Type type when type == typeof(ConstructorInfo):
                    _memberType = Unsafe.As<Func<TMember, Type>>((Func<ConstructorInfo, Type>)GetConstructorDeclaringType);
                    break;

                case Type type when type == typeof(MethodInfo):
                    _memberType = Unsafe.As<Func<TMember, Type>>((Func<MethodInfo, Type>)GetMethodDeclaringType);
                    break;

                default:
                    _memberType = (TMember _) => throw new NotImplementedException();
                    break;
            }
        }

        public ImportInfo(TMember info)
        {
            _info = info;
            IsImport = default;
            ValueData = default;
        ...
        private ImportInfo(ref ImportInfo<TMember> parent, Type type, object? data)
        {
            _info = parent._info;
            IsImport = false;
            AllowDefault = false;
        ...
        public ImportInfo<TMember> With(Type type, object? value)
            => new ImportInfo<TMember>(ref this, type, value);
