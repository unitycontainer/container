        ...
        internal struct Recorder
        {
            ...
            public bool Add(in Contract contract)
            {
                ...
                while (position > 0)
                {
                    ...
                    if (contract.HashCode == entry.Next && 0 == entry.Position)
                        return false;
            ...
            public bool Add(int level, int index, in Contract contract)
            {
                ...
                while (position > 0)
                {
                    var scope = _ancestry[_data[position].Next]._scope;
            ...
            private void Expand()
            {
                int prime = Prime.IndexOf(_meta.Length);
                ...
                for (var position = 1; position <= _index; position++)
                {
                    ...
                    var scope = _ancestry[record.Next]._scope;
