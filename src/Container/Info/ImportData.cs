    ...
    { 
        public static ImportData AsImportData<T>(this T info, object? data)
        {
            return data switch
            {
                ...
                {
                    FieldInfo field                       => new ImportData(typeFactory.GetResolver<PipelineContext>(field.FieldType),         ImportType.Pipeline),
                    PropertyInfo property                 => new ImportData(typeFactory.GetResolver<PipelineContext>(property.PropertyType),   ImportType.Pipeline),
                    ParameterInfo parameter               => new ImportData(typeFactory.GetResolver<PipelineContext>(parameter.ParameterType), ImportType.Pipeline),
                    _                                     => new ImportData(data,                                                              ImportType.Unknown),
                },
