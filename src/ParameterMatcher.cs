...
namespace Unity
{
    ...
    public class ParameterMatcher
    {
        ...
        private readonly List<InjectionParameterValue> _parametersToMatch;
        ...
        {
            _parametersToMatch = new List<InjectionParameterValue>(parametersToMatch);
        }
        ...
        public virtual bool Matches(IEnumerable<Type> candidate)
        {
            ...
            if (_parametersToMatch.Count == candidateTypes.Count)
            {
                for (int i = 0; i < _parametersToMatch.Count; ++i)
                {
                    if (!_parametersToMatch[i].MatchesType(candidateTypes[i]))
                    {
                        return false;
                    }
                }
